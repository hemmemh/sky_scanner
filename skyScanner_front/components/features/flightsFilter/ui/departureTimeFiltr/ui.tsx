...
export const DepartureTimeFiltr = memo(() => {
  ...
  const [rangeDepartureTime, setRangeDepartureTime] = useState<number[]>([
    0, 0,
  ]);
  ...
  useEffect(() => {
    const departTimeParam = searchParams.get("departureTimeFiltr")
      ? searchParams.get("departureTimeFiltr")?.split("%2C")
      : [];
  ...
  const onDepartureTimeViewChange = (
    event: Event | SyntheticEvent<Element, Event>,
    value: number | number[],
  ) => {
