...
import { selectPage } from "@/components/entities/TripList";
...
export const StopsFiltr = memo(() => {
  ...
  useEffect(() => {
    ...
    if (!uncheckedStopsFirstUpdate.current) {
      if (uncheckedStops.length > 0) {
        currentParams.set(
          "stops",
          uncheckedStops.map((el) => el.value).join("%2C"),
        );
  ...
  useEffect(() => {
    const stopsParam: StopValue[] | undefined = searchParams.get("stops")
      ? (searchParams.get("stops")?.split("%2C") as StopValue[])
      : [];

    if (
      stopsParam &&
      stopsParam.length !== 0 &&
      uncheckedStopsFirstUpdate.current
    ) {
    ...
    <div className={styles.stops}>
      {stopsDefault.map((el) => (
        <FormControlLabel
          ...
          control={
            <Checkbox
              checked={isValueInStop(el.value)}
              onChange={() => onStopChange(el)}
              name={el.name}
            />
          }
        />
