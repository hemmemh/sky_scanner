  ...
  const addToOrder = useCallback(()=>{
    ...
    if (user && trips) {
      if (isTripsPairs(trips)){
        ...
        useDispatch(addOrderAction({ user, from:trips[0], to:trips[1], to_positions, from_positions })).then(()=>{
          setSnackBarOpen(true)
        ...
        useDispatch(addOrderAction({ user, from:trips, from_positions })).then(()=>{
          setSnackBarOpen(true)
          setSnackBarMessage('Успешное офромление билета')
        }).catch(error=>{
          setSnackBarOpen(true)
