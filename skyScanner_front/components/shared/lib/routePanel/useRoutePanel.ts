...
import { fetchTripListForCalendar } from "@/components/entities/TripList/model/tripListThunk";
import {
  selectCalendarTrip,
  selectCalendarTripsLoading,
} from "@/components/entities/TripList/model/selectors";
...
export const UseRoutePanel = () => {
  ...
  const [validationState, setValidationState] =
    useState<InfoValidation>(validationDefault);
  ...
  const calendarTrips = useAppSelector(selectCalendarTrip);
  const calendarTripsLoading = useAppSelector(selectCalendarTripsLoading);
  ...
  useEffect(() => {
    ...
    if (info.from !== "" && info.to !== "" && depart) {
      useDispatch(
        fetchTripListForCalendar({
          query: info,
          params: { depart: depart, return: depart },
        }),
      );
    }
  }, [info]);
  ...
  const onInfoChange = (
    type: panelInputLabel,
    value: string | number | ISeatClass,
  ) => {
