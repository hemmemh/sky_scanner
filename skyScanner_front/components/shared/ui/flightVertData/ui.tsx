...
export const FlightVertData: FC<IFlightVertData> = memo(({ trip }) => {
  const { start, end, hours, minutes } = useMemo(
    () => msToHoursAndMinutes(+trip.departure_time, +trip.arrival_time),
    [trip],
  );
    ...
    <div className={styles.body}>
      ...
      <div className={styles.info}>
        ...
        <div className={styles.data}>
          {start} {sliceCity(trip.departure_city.name[lang])}{" "}
          {trip.departure_city.name[lang]}
        </div>
        <div className={styles.data}>
          {end} {sliceCity(trip.arrival_city.name[lang])}{" "}
          {trip.arrival_city.name[lang]}
        </div>
      </div>
    </div>
  );
