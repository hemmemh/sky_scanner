...
export const SeatPicker: FC<ISeatPicker> = ({
  className,
  label,
  placeholder,
  onChange,
  ...
  const changeSeatClass = (event: SelectChangeEvent) => {
    const item = seatClassList.find(
      (el) => el.name[lang] === (event.target.value as string),
    );
    if (item) {
      setSeatClass(item);
    }
  ...
  return (
    <div
      ref={autocompleteRef}
      onClick={handleOpen}
      className={clsx(styles.case, className, { [styles.active]: popperOpen })}
    >
      <div className={styles.case__body}>
        ...
        <div>
          ...
          <div
            ref={popperRef}
            className={clsx(styles.popper, { [styles.visible]: popperOpen })}
          >
            <div className={clsx(styles.popper__body)}>
              ...
              {seatClass && (
                <FormControl fullWidth>
                  <Select
                    ...
                    defaultValue="dd"
                  >
                    {seatClassList.map((el) => (
                      <MenuItem key={el.uid} value={el.name[lang]}>
                        {el.name[lang]}
                      </MenuItem>
                    ))}
                  </Select>
              ...
              <div className={styles.seats}>
                <Title marginBottom="10px" color="black">
                  Места
                </Title>
                <NumberInput
                  min={1}
